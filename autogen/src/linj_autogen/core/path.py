...
class PathResolver:
    ...
    def parse(cls, path: str) -> List[PathSegment]:
        """
        Parse path string to path segment list
    ...
    def get(cls, obj: Any, path: str) -> Any:
        ...
        Section 5.2: Return None (null value) for non-existent paths

        Args:
            obj: Root object (usually dict)
            path: Path string

        Returns:
            Value at path, or None if not exists
        """
        try:
    ...
    def set(
        ...
        Section 5.3:
        - Auto-create {} when intermediate object is missing
        - Auto-create [] when intermediate array is missing
        ...
        for seg in segments[:-1]:
            next_seg = segments[segments.index(seg) + 1]
    ...
    def delete(cls, obj: dict, path: str) -> None:
        ...
        Section 5.4:
        - Path returns null after deletion
        - Deleting non-existent field is no-op (no error)
        - Set to null when deleting array element (array not shortened)
        ...
        if last_seg.is_array_index():
            ...
            if isinstance(parent, list) and 0 <= idx < len(parent):
                parent[idx] = None  # Set to null, array not shortened
        else:
            key = last_seg.key
    ...
    def intersect(cls, path_a: str, path_b: str) -> bool:
        ...
        - One path is prefix of another ($.a vs $.a.b)
        ...
        for i in range(min_len):
            ...
            if seg_a.key != seg_b.key:
                # Same position but different keys -> not intersect
                return False
    ...
    def get_parent_and_key(cls, path: str) -> tuple:
        """
        Get parent path and final key of path
