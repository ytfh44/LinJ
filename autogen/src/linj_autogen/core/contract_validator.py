    ...
    def raise_if_invalid(self, node_id: str) -> None:
        ...
        if not self.valid:
            ...
            raise ContractViolation(
                f"Contract validation failed for node {node_id}",
                error_details
            )
...
class ContractValidator:
    ...
    def validate(
        ...
        if contract.type not in self.SUPPORTED_TYPES:
            ...
            self.logger.warning(
                "Unsupported contract type '%s' at path '%s'",
                contract.type, path
            )
            return result
    ...
    def validate_in_contract(
        ...
        self.logger.debug(
            "Validating in_contract for node '%s'", node.id
        )
    ...
    def validate_out_contract(
        ...
        self.logger.debug(
            "Validating out_contract for node '%s'", node.id
        )
