...
from ..core.errors import ConflictError, LinJError
...
class CommitManager:
    ...
    def submit(
        self, step_id: int, base_revision: int, changeset: ChangeSet, handle: str
    ) -> CommitResult:
        """
        提交变更集
    ...
    def can_accept(self, step_id: int, changeset: ChangeSet) -> bool:
        """
        检查变更集是否可以被接受
    ...
    def _check_intersection_with_pending(
        self, step_id: int, changeset: ChangeSet
    ) -> bool:
        """
        ...
        for other_step_id, pending in self._pending.items():
            if other_step_id < step_id and pending.status == PendingStatus.PENDING:
                if changeset.intersects_with(pending.changeset):
                    return True
    ...
    def _check_intersection_since_revision(
        self, base_revision: int, changeset: ChangeSet
    ) -> bool:
        """
        ...
        for step_id, result in self._results.items():
            if (
                ...
                if step_id in self._pending:
                    ...
                    if changeset.intersects_with(other_cs):
                        return True
        return False
