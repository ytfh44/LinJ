...
from ..core.errors import HandleExpired, ConflictError
...
class ContiTextEngine:
    ...
    def __init__(self, state_manager: Optional[StateManager] = None):
        ...
        self._pending_changes: Dict[int, ChangeSet] = {}
    ...
    def suspend(
        ...
        if cont.handle not in self._continuations:
            raise HandleExpired(f"Continuation {cont.handle} not found")
    ...
    def resume(self, handle: str, input_data: Optional[Dict] = None) -> Continuation:
        ...
        if handle not in self._continuations:
            raise HandleExpired(f"Continuation {handle} expired or invalid")
    ...
    async def join(self, handles: List[str]) -> List[JoinResult]:
        ...
        for handle in handles:
            if handle not in self._continuations:
                raise HandleExpired(f"Continuation {handle} not found")
    ...
    def complete(self, handle: str, result: Any) -> None:
        ...
        if cont.status == Status.CANCELLED:
            raise ConflictError("Cannot complete cancelled continuation")
    ...
    def fail(self, handle: str, error: str) -> None:
        ...
        if cont.status == Status.CANCELLED:
            raise ConflictError("Cannot fail cancelled continuation")
    ...
    def create_view(
        self, cont: Continuation, pending_changes: Optional[List[ChangeSet]] = None
    ) -> ContinuationView:
        """
        为续体创建视图 (18.2 节)
